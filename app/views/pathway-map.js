/* Qualification pathway map — full screen view */
...
export function PathwayMapView({ backRoute = 'chat-first', backSlices = {} } = {}) {
  const el = document.createElement('div');
  el.className = 'view view-pathway-map';
  ...
  title.textContent = 'Qualification pathways';
  content.appendChild(title);

  const intro = document.createElement('p');
  intro.className = 'cd-body pathway-screen-intro';
  intro.textContent = 'Tap any qualification to find out more.';
  content.appendChild(intro);

  const mapWrap = document.createElement('div');
  mapWrap.className = 'pathway-map-wrap';
  mapWrap.innerHTML = `
  ...
  expPanel.className = 'pathway-explanation';
  expPanel.hidden = true;
  content.appendChild(expPanel);
  ...
  function renderExplanation(nodeKey) {
    ...
    el.querySelectorAll('.pathway-node--active').forEach(n => n.classList.remove('pathway-node--active'));
    el.querySelector(`[data-node="${nodeKey}"]`).classList.add('pathway-node--active');
    activeNode = nodeKey;
  ...
  el.querySelectorAll('.pathway-node').forEach(node => {
