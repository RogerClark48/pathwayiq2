"""
cluster_pathway_refinement_test2.py
Re-test cluster pathway enrichment on clusters 2 and 11 with refined prompt
that adds a clear statement of purpose before the track rules.
...
OUTPUT_TXT = r"C:\Dev\pathwayiq2\scripts\cluster_pathway_refinement_test2_output.txt"
MODEL      = "claude-haiku-4-5-20251001"

TEST_CLUSTER_IDS = ["2", "11"]
...
SYSTEM_PROMPT = (
    ...
    "What a track is for:\n"
    "A track represents a coherent career journey that a student could actually follow — a "
    "recognisable path from a starting point to a destination, with a clear sense of what you "
    "would be doing along the way. The test of a good track is whether a student could read it "
    "and say \"yes, that is the direction I want to go.\" A track that is too narrow (one "
    "specialism, two jobs) does not give a student enough to orient by. A track that is too "
    "broad (all scientists lumped together) does not help them choose. The right level of "
    "granularity is one where the entry route, the day-to-day work, and the senior destination "
    "are meaningfully shared across all the jobs in the track.\n\n"
    "Rules for identifying tracks:\n\n"
    "- Identify tracks by entry route, progression structure, and career destination — not by "
    "scientific or technical discipline. Two roles that require the same type of qualification "
    "to enter, progress through similar levels of seniority, and lead to similar senior "
    "destinations belong in the same track even if they are in different scientific or "
    "technical fields.\n"
    "- Only create separate tracks where entry requirements, progression logic, or career "
    "destinations are genuinely different.\n"
...
def main():
    ...
    clusters = db.execute(
        f"SELECT cluster_id, name FROM clusters WHERE cluster_id IN "
        f"({','.join('?' * len(TEST_CLUSTER_IDS))}) "
        f"ORDER BY cluster_id + 0, cluster_id",
        TEST_CLUSTER_IDS,
    ).fetchall()
