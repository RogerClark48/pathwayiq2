"""
cluster_pathway_production.py
Cluster pathway enrichment — all 21 clusters.
Uses Anthropic tool-use API for structured output.
Writes to staging tables in job_roles_asset.db.
"""
...
OUTPUT_TXT  = r"C:\Dev\pathwayiq2\scripts\cluster_pathway_production_output.txt"
...
def ensure_staging_tables(db):
    db.executescript("""
        CREATE TABLE IF NOT EXISTS cluster_pathway_staging (
        ...
        CREATE TABLE IF NOT EXISTS track_jobs_staging (
            id               INTEGER PRIMARY KEY AUTOINCREMENT,
...
def load_all_clusters(db):
    return db.execute(
        "SELECT cluster_id, name FROM clusters ORDER BY cluster_id + 0, cluster_id"
    ).fetchall()


# Set to a list of cluster_ids to re-run specific clusters only, or None for all.
RERUN_ONLY = None  # Set to e.g. ["7", "13"] to re-run specific clusters only
...
def write_staging(db, cluster_id, cluster_name, result, generated_at):
    # cluster_pathway_staging — upsert
    db.execute(
        """
    ...
    # Delete any existing tracks for this cluster (re-run safety)
    old_track_ids = [
...
def main():
    ...
    ensure_staging_tables(db)

    client   = anthropic.Anthropic()
    all_clusters = load_all_clusters(db)
    clusters = [c for c in all_clusters if RERUN_ONLY is None or c["cluster_id"] in RERUN_ONLY]

    succeeded = []
    failed    = []

    txt_mode = "a" if RERUN_ONLY else "w"
    with open(OUTPUT_TXT, txt_mode, encoding="utf-8") as txt_out:
    ...
    print(f"\n{'='*50}")
    print(f"Complete: {len(succeeded)}/{len(clusters)} clusters succeeded")
    if failed:
        print(f"Failed ({len(failed)}):")
        for cid, name, err in failed:
            print(f"  Cluster {cid} ({name}): {err}")
    print(f"Staging tables written to: {JOBS_DB}")
    print(f"Text output written to:    {OUTPUT_TXT}")
