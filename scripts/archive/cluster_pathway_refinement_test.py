"""
cluster_pathway_refinement_test.py
Re-run cluster pathway enrichment on 4 clusters with refined prompt.
...
OUTPUT_TXT = r"C:\Dev\pathwayiq2\scripts\cluster_pathway_refinement_test_output.txt"
MODEL      = "claude-haiku-4-5-20251001"

TEST_CLUSTER_IDS = ["2", "4", "11", "13"]
...
SYSTEM_PROMPT = (
    ...
    "- Each track must contain at least 3 jobs. If a natural grouping has only 1 or 2 jobs, "
    "fold it into the closest related track rather than creating a thin standalone entry.\n"
...
def main():
    ...
    clusters = db.execute(
        "SELECT cluster_id, name FROM clusters WHERE cluster_id IN (?, ?, ?, ?) "
        "ORDER BY cluster_id + 0, cluster_id",
        TEST_CLUSTER_IDS,
    ).fetchall()
